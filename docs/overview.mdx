...
Unforgettable exists because both seed phrases and MPC wallets protect keys at the cost of either human usability or true self-custody.

**Seed phrases** are cryptographically robust but human-hostile. They require users to permanently store a high-value secret, creating constant risk of loss, theft, phishing, or accidental exposure. Once leaked, compromise is total and irreversible. This model does not scale to mainstream users.

**MPC wallets** eliminate seed phrases but introduce infrastructure dependency. Keys are split across providers and services that must remain online, honest, and compliant. This results in soft custody, regulatory ambiguity, higher operational costs, and systemic risks such as DoS or collusion. The user no longer fully controls the key lifecycle.

**Unforgettable takes a third path.** It removes both persistent secrets and custodial signers by deriving keys just-in-time on the user’s device from user-held factors (e.g., biometrics and visual keys) using fuzzy extractors. This enables:

* **No seed phrases:** Users authenticate via visual memory (face, favorite object, etc.) instead of managing 12–24-word phrases.
* **Self-custodial and private:** The signing key is re-derived locally; only a public key and helper data are stored on-chain or on the app server. Raw biometrics and key material never leave the device.
* **Multi-factor security:** Visuals (e.g., face + object), location, and optional password or proof-of-work (PoW) combine to reach a full 112-bit security target.
* **Smooth onboarding:** Setup takes seconds, making it suitable for mainstream dApps.
* **Phishing resistance:** There is nothing to type or copy, eliminating most social-engineering vectors.
